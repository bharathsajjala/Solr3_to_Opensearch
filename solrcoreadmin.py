...
class SolrCoreAdmin(object):
    ...
    def get_all_cores(self, name=None):
        if name:
            status_cmd = self.core_admin_url + urlencode({'action':'status','core':name,'wt':'json'})
        else:
            status_cmd = self.core_admin_url + urlencode({'action':'status','wt':'json'})
        
        res =  request.urlopen(status_cmd)
        ...
        result ={}
        for x in cores:
           numdoc = data['status'][x]["index"]["numDocs"]
           result[x] = numdoc
        return result    
    ...
    def create_core(self,name):
        ...
        create_cmd = self.core_admin_url +urlencode({'action':'create','name':name,'loadOnStart':'false', 'instanceDir':'.','schema':'schema.xml'})
        res =  request.urlopen(create_cmd)
    ...
    def unload_core(self,name,delete_index=False):
        ...
        unload_cmd = self.core_admin_url + urlencode(params)
        res =  request.urlopen(unload_cmd)
